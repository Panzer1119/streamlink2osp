...
| `OSP_DELETE_HLS_FILES_AFTER_STREAM_END` | Deletes HLS files of the stream after the stream has ended. This also checks if the current stream is the same as before and if not removes the files. **Enables `TWITCH_ENABLE_API` automatically.** **Requires the `tempfs` directory to be mounted at `/tempfs`.** | `false`                                                                     | `true`                          |
